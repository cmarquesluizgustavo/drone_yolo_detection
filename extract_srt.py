...
def extract_first_values_from_srt(srt_path: str) -> dict:
    ...
    # Extract first GPS coordinates: GPS(W: lon, S: lat, altitude)
    gps_pattern = r'GPS\(W:\s*([-\d.]+),\s*S:\s*([-\d.]+),\s*[-\d.]+m?\)'
    gps_match = re.search(gps_pattern, content)
    
    if gps_match:
        result['lon'] = float(gps_match.group(1))
        result['lat'] = float(gps_match.group(2))
